...
Реши, является ли ответ грамматически корректным И уместным решением задания — даже если он не совпадает с образцом дословно.
Допустимы: другой порядок слов, синонимичные формулировки, опущенное подлежащее, отсутствие запятых.
Ошибки: неверное спряжение, неверный предлог/артикль, пропущенная диакритика, слова не по-португальски, ответ не о том.
Отвечай ТОЛЬКО JSON: {"ok": true/false, "why": "одно короткое предложение по-русски", "fix": "исправленный вариант ответа ученицы (если есть ошибки, иначе пустая строка)"}`;
